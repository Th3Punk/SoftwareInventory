...
- Hitelesítés: lokális (DB-alapú) felhasználó/jelszó, külső függőség nélkül; bővíthető provider rendszerrel (AD/Kerberos későbbi fázisban)
- Jogosultságkezelés: RBAC, induláskor manuális szerepkör-hozzárendeléssel (csoport-leképzés a külső providerekkel érkezik)
...
- AD/Kerberos SSO + LDAP csoportleolvasás (későbbi fázis – Fázis 4)
...
| ------------------- | ----------------------- | --------------------------- | -------------------------------------------------------- |
| Authentikáció       | `IAuthProvider`         | `LocalAuthProvider`         | `KerberosAuthProvider`, `OAuthProvider`, `OktaProvider`  |
| Csoportleolvasás    | `IGroupProvider`        | `NullGroupProvider`         | `LdapGroupProvider`, `OktaGroupProvider`, `ClaimsGroupProvider` |
...
public class ExternalIdentity
{
    ...
    public required string ExternalId { get; set; }         // Local: username; AD: sAMAccountName
    public string? ExternalGroupsJson { get; set; }         // cachedelt csoportlista (külső providereknél)
    public DateTime LastSyncedAt { get; set; }
    public User User { get; set; } = null!;
}

// Lokális (DB-alapú) hitelesítés jelszó-adatai – csak ProviderType=Local esetén (1. fázis).
// Külön entitásban, hogy a provider-specifikus adat ne szennyezze a közös modellt.
public class LocalCredential
{
    public int Id { get; set; }
    public int UserId { get; set; }
    public required string PasswordHash { get; set; }       // PBKDF2 (ASP.NET Core PasswordHasher), per-user salt
    public bool MustChangePassword { get; set; } = true;    // bootstrap admin / reset után
...
public interface IAuthProvider
{
    ...
    // Local: hitelesítő adatok ellenőrzése; Kerberos/Negotiate: middleware trigger; OAuth: redirect URL
...
### 7.2 Lokális (DB-alapú) hitelesítés – 1. fázis

Az **induló** authentikációs implementáció szándékosan **nem támaszkodik külső rendszerre** (nincs Active Directory / KDC / LDAP függőség). A felhasználók és a jelszó-hash-ek a saját adatbázisban élnek, a szerepkörök az alkalmazáson belül, manuálisan kerülnek hozzárendelésre (`UserRole`, `Source = Manual`). Így a rendszer önállóan, külső infrastruktúra-előfeltétel nélkül üzembe helyezhető.

> A pluggable architektúra (P1) miatt a későbbi AD/Kerberos vagy OAuth bevezetése **konfigurációs váltás** (`Features:Authentication:Provider`), az üzleti logika érintése nélkül.
...
**Tárolás és hash:** a jelszó-hash a `LocalCredential` táblában (ASP.NET Core `PasswordHasher<T>`, PBKDF2, per-user salt). Plain szöveg sosem kerül tárolásra vagy logba (16.3).

**Működési folyamat:**

```
1. POST /api/v1/auth/login { username, password }
2. LocalAuthProvider: ExternalIdentity (ProviderType=Local) keresése + jelszó-hash ellenőrzés
3. Sikertelen kísérlet → FailedAttempts++; MaxFailedAttempts felett ideiglenes lockout (LockedUntil)
4. Siker: szerepkörök betöltése a UserRole táblából (Source=Manual)
5. Cookie-alapú munkamenet (HttpOnly, Secure, SameSite=Lax), ClaimsPrincipal a szerepkörökkel
6. MustChangePassword esetén kötelező jelszócsere a folytatás előtt
```

**Bootstrap admin:** első indításkor egy kezdeti `Admin` felhasználó jön létre (migration seed; induló jelszó környezeti változóból/secretből, `MustChangePassword = true`). Külső csoport-forrás híján e nélkül a rendszer nem lenne adminisztrálható.

**Auth végpontok (Local):**

```
POST /api/v1/auth/login            → bejelentkezés (munkamenet-cookie)
POST /api/v1/auth/logout           → kijelentkezés
GET  /api/v1/auth/me               → aktuális felhasználó + szerepkörök
POST /api/v1/auth/change-password  → saját jelszó csere
```

A felhasználó-létrehozást és a szerepkör-hozzárendelést az admin felület kezeli (ld. 9/RBAC, admin modul).

### 7.3 AD/Kerberos + LDAP provider – későbbi fázis

> **Megjegyzés:** Ez a provider **későbbi fázisban** készül el (ld. 17. ütemterv, Fázis 4). Az 1. fázisban a 7.2 lokális hitelesítés az aktív. Az alábbi terv változatlanul érvényes, amikor a szervezet az AD/Kerberos SSO-t bevezeti; a bekapcsolás `Features:Authentication:Provider = "Kerberos"` átállítással történik.
...
// NullGroupProvider (1. fázis, Local auth): nincs külső csoport-forrás,
//   a szerepkörök közvetlenül a UserRole táblából jönnek (Source=Manual)
// LdapGroupProvider (későbbi fázis): LDAP bind → memberOf attribútum olvasás + cache
// OktaGroupProvider (jövőbeli): Okta Groups API + cache
// ClaimsGroupProvider (jövőbeli): JWT token claims-ből olvas
```

### 7.5 További providerek előkészítése

Az `IAuthProvider` és `IGroupProvider` interfészek tervezetten fogadják a további (későbbi fázisú) implementációkat:

| Provider                       | Fázis      | Auth flow                          | Csoport-forrás       |
| ------------------------------ | ---------- | ---------------------------------- | -------------------- |
| `LocalAuthProvider`            | **1. (aktív)** | Username/password (cookie session) | `UserRole` (manuális) |
| `KerberosAuthProvider` + `LdapGroupProvider` | Fázis 4    | Negotiate / SPNEGO                 | LDAP `memberOf`      |
| `OAuthProvider`                | Fázis 4    | OIDC authorization code flow       | JWT `groups` claim   |
| `OktaProvider`                 | Fázis 4    | Okta OIDC                          | Okta Groups API      |

A `GroupRoleMapping.ProviderType` oszlop biztosítja, hogy a különböző (külső) providerektől érkező csoportok eltérő leképzésekkel rendelkezzenek. A lokális provider nem használ csoport-leképzést: a szerepkörök közvetlenül, manuálisan kerülnek hozzárendelésre.
...
- Authentication: cookie-alapú munkamenet lokális bejelentkezés után (1. fázis); későbbi fázisban Windows Negotiate (Kerberos) is választható ugyanazon `IAuthProvider` mögött
...
**1. fázis (lokális):** bejelentkező űrlap → `POST /api/v1/auth/login`, a szerver `HttpOnly` munkamenet-cookie-t állít ki. Minden további kérés `credentials: 'include'` opcióval megy, hogy a cookie csatolódjon. A `useAuth` hook a `GET /api/v1/auth/me` alapján tartja a munkamenet-állapotot, és kezeli a kötelező jelszócserét (`MustChangePassword`).
...
// Hitelesített kérés (a cookie automatikusan csatolódik)
...
> Későbbi fázisban (Kerberos/Negotiate) ugyanez a `credentials: 'include'` minta marad érvényben; a böngésző a Negotiate ticketet automatikusan küldi, bejelentkező űrlap nélkül.
...
> **Fázis-megjegyzés:** A `libgssapi-krb5-2` / `krb5-user` runtime csomagok csak a Kerberos provider fázisához (Fázis 4) szükségesek. Az 1. fázisú lokális hitelesítéshez nem kellenek; ekkor a `base` réteg ezen `RUN` lépése elhagyható. Hasonlóan, a 15.1 Docker Compose `ldap-password` secretje is csak a Kerberos/LDAP fázisban releváns.

```dockerfile
FROM mcr.microsoft.com/dotnet/aspnet:9.0-bookworm-slim AS base
# Kerberos Linux runtime függőség (csak a Kerberos provider fázisához – Fázis 4)
...
### 15.4 Kerberos infrastruktúra előfeltételek (Fázis 4)

> Ezek az előfeltételek **kizárólag az AD/Kerberos provider fázisához** tartoznak. Az 1. fázisú lokális hitelesítés egyik elemet sem igényli.
...
- Minden API endpoint hitelesített kérést vár (kivétel: `/health`, `/api/v1/config/features`, `/api/v1/auth/login`)
- **1. fázis (lokális):** jelszó-hash PBKDF2-vel (`PasswordHasher<T>`), per-user salt; plain jelszó sosem tárolódik/logolódik. Jelszó-policy és fiók-lockout (`MaxFailedAttempts`). Munkamenet `HttpOnly`+`Secure`+`SameSite` cookie-val; a session-védelem az ASP.NET Core Data Protection API-ra épül
- A bootstrap admin induló jelszava Secret/environment változóból jön, és első belépéskor kötelezően cserélendő
- **Későbbi fázis (Kerberos/LDAP):** a keytab fájl soha nem kerül a Docker image-be (csak mount/Secret); az LDAP service account jelszava Kubernetes Secret-ben van, nem environment változóban
...
- [ ] Lokális (DB-alapú) AuthProvider: felhasználó/jelszó, cookie session, bootstrap admin (külső függőség nélkül)
- [ ] Alapvető RBAC middleware + manuális szerepkör-hozzárendelés (`NullGroupProvider`)
- [ ] Application + Environment + Contact + Tag CRUD API (forráskód-link + web-környezet linkek mezőkkel)
- [ ] React frontend: bejelentkezés, alkalmazás-lista és részletes nézet
...
- [ ] AD/Kerberos AuthProvider + LDAP GroupProvider (külső SSO bevezetése) + GroupRoleMapping admin használatba vétele
...
| Fogalom              | Definíció                                                                                                                |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **AuthProvider**     | Az authentikációs logikát absztraháló interfész; minden bejelentkezési mód ennek implementációja. 1. fázis: `LocalAuthProvider` (DB-alapú, külső függőség nélkül); későbbi: Kerberos, OAuth, Okta |
| **LocalAuthProvider** | Az 1. fázisú, DB-alapú felhasználó/jelszó hitelesítés; nincs külső rendszer-függősége, a szerepkörök manuálisan kerülnek hozzárendelésre                |
